...
UPDATE tblkaryawan set tblkaryawan.[shift]=Tblshifbulan.[23],tblkaryawan.[Status Shift]='Y' from tblkaryawan   --merubah status shift
		...
		WHERE Tblshifbulan.tahun='2024' and Tblshifbulan.bulan='07' and Tblshifbulan.[23] > '0'


/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	*/
/* UNTUK MENCARI SUMMARY KARYAWAN ALL SHIFT			( langsung execute ) adm kye tok (hari sebelumnya)																																			*/
/*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/   
...
FROM tblattendance AS TA WITH(NOLOCK)
	   ...
	   where TA.tahun=2024 and TA.bulan='07'
			 --AND TK.Shift IN ('0','1')	 /* SHIFT 1 YAAA DI   */
			 --AND tk.Shift = '3'			 /* SHIFT 2 DAN 3 DI  */
			 AND (TK.Resign = 'N' OR TK.[Resign Date] > '2024-07-23')
             AND CONVERT(VARCHAR(10), TK.[Tgl. Kerja Lama], 120) <= '2024-07-23'
	   order by TA.nik

/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	*/
/* UNTUK MENCARI PER DEPARTMENT					( ubah shift sesuai kebutuhan )																																				*/
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	*/
...
WHERE TA.Sts23 IN ('H','A','I','D','S','Z','L','C','T','U','V','W','X','Y','O','')/*SESUAIKAN DENGAN TANGGAL SEKARANG (sts01)*/
		AND TA.BULAN = '07'
		AND TA.TAHUN = 2024
		--AND TK.Shift IN ('0','1')     /* INI JIKA INGIN MENGAMBIL SHIFT 1	*/
		AND TK.Shift = '3'				/* INI JIKA INGIN MENGAMBIL SHIFT 2 ATAU SHIFT 3	*/
		AND (TK.Resign = 'N' OR TK.[Resign Date] > '2024-07-23')
        AND CONVERT(VARCHAR(10), TK.[Tgl. Kerja Lama], 120) <= '2024-07-23'
GROUP BY TD.[Nama Departemen] ORDER BY [Nama Departemen]

/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	*/
/* UNTUK MENCARI PER BAGIAN					( langsung execute )																																				*/
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	*/
