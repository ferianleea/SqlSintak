...
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	*/
/* UNTUK MENCARI SUMMARY KARYAWAN SHIFT 1	( LANGSUNG EXECUTE )	 adm kye tok (hari ini)																								*/
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	*/
SELECT TA.nik,TK.nama,TD.[Nama Departemen],TB.[Nama Bagian],TB2.[Nama Bagian2],TJ.Jabatan,kategori,
       TA.[in23],TA.[sts23],/* SESUAIKAN DENGAN TANGGAL SEKARANG (sts01) */
...
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	*/
/* UNTUK MENCARI PER DEPARTMENT			( LANGSUNG EXECUTE )																															*/
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	*/
...
WHERE TA.sts23 IN ('H','A','I','D','S','Z','L','C','T','U','V','W','X','Y','O','')/*SESUAIKAN DENGAN TANGGAL SEKARANG (sts01)*/
		...
		AND (TK.Resign = 'N' OR TK.[Resign Date] > '2024-07-23')
        AND CONVERT(VARCHAR(10), TK.[Tgl. Kerja Lama], 120) <= '2024-07-23'
GROUP BY TB.[Nama Bagian],TD.[Nama Departemen] ORDER BY [Nama Departemen]
